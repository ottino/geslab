...
class ReportesController extends AppController{
    ...
    public function search_facturacion() {
        ...
        $filtro = $this->request->data['Reporte']['periodo'];
    ...
    public function search_rendicion() {
        ...
        $data = $this->Reporte->query_reporte_5($fecha_ini , $fecha_fin);
        $this->set('data' ,$data);
        return $this->render('rendicion','ajax');

      }   

    public function reporteiosper() {


      }

    public function reportefemer() {
       $this->set('obrasociales'      , $this->obrasociales);

      }

    public function reporteFacturacioniosper ($id = null , $modo = 0 , $logo = 2 , $fecha_periodo = null) {
    ...
    public function reporteFacturacionfemer ($id = null , $modo = 0 , $logo = 2 , $fecha_periodo = null) {
                ...
                # cursor que va protocolo por protocolo
                 ...
                 $fecha_periodo_ini = $this->request->data['Reporte']['periodo_ini'];
                 $fecha_periodo_fin = $this->request->data['Reporte']['periodo_fin'];
                 $nro_protocolo     = 0;//$this->request->data['Reporte']['protocolo_nro'];
                 $listadosos        = implode(",",$this->request->data['Reporte']['lsitaOS']);

                 $base_inicial  = $this->Reporte->query_reporte_6($fecha_periodo_ini , $fecha_periodo_fin, $nro_protocolo, $listadosos);
                ...
                {    
                 ...
                 $data = $this->Reporte->query_reporte_7($fecha_periodo , $id);
                 ...
                 // Cupon para IOSPER *****
                  ...
                  $pdf->SetX(3.5);
                  //$pdf->Cell(3.5,0.22,'I . O . S . P . E . R');    
                  $pdf->Image(IMAGES . 'logo_femer.jpg',$pdf->GetX(),$pdf->GetY()-0.5,3,2.5);              
                  $pdf->SetXY(7.8,$pdf->GetY()+0.65);
                  $pdf->Cell(3.5,0.22,'Fecha Ingreso:  ' . $cup_iosper_fingreso);
                  $pdf->SetX(14);
                  $pdf->Cell(3.5,0.22,'DNI ' . $cup_iosper_dni);                                   
                 ...
                 switch (count($data)) {
                    ...
                    $data_ = $pdf->Output(null , 'S');
                    $this->set('id', 'FacturacionFEMER'.$numero_listado);
                    $this->set('pdf_comprobantes', $data_);
                    $this->render('reporte_facturacionfemer', 'ajax');
    }          
